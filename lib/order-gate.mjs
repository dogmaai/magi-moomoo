 ...
 *     The legacy `source='magi-core'` request-body label is spoofable and is
 *     disabled by default; it is honored only when the operator explicitly
 *     opts in via `allowLegacySourceLabel` (env GATE_ALLOW_LEGACY_SOURCE)
...
const APPROVALS_TABLE = '`screen-share-459802.magi_core.order_approvals`';
 ...
 * @param {number} [deps.killSwitchTtlMs] - kill-switch read cache window
 *   (default 5s). An emergency halt can take up to this long to take effect;
 *   a shorter window bounds that delay at the cost of a LIMIT-1 BQ read.
 * @param {boolean} [deps.allowLegacySourceLabel] - accept the spoofable
 *   `source='magi-core'` body label when no trusted-caller allowlist is
 *   configured. Default false: without an allowlist every non-reducing order
 *   requires an approval token.
 */
export function createOrderGate({ bqQuery, getPositions, trustedCallerEmails = [], verifyIdToken = null, now = () => Date.now(), killSwitchTtlMs = 5_000, allowLegacySourceLabel = false }) {
  let _cache = null;
  let _cacheAt = 0;
  ...
  /**
   * Fail-closed helper: a confirmed HALTED stays latched across unreadable
   * reads (an outage cannot have cleared it — /resume writes through the
   * same table); anything else degrades to UNKNOWN.
   */
  function unknownOrLatched(why) {
    if (_lastConfirmed?.state === 'HALTED') {
      console.warn(`[GATE] Kill-switch ${why}; confirmed HALTED stays latched`);
      return _lastConfirmed;
    }
    console.warn(`[GATE] Kill-switch ${why} — UNKNOWN (reduce-only)`);
    return { state: 'UNKNOWN', reason: null, updatedBy: null, updatedAt: null };
  }

  async function killSwitchState() {
    const t = now();
    if (_cache && t - _cacheAt < killSwitchTtlMs) return _cache;
    try {
      ...
      // Only an explicit boolean confirms state. 0 rows, NULL, or a
      // non-boolean value is NOT evidence of RUNNING — it means the control
      // table cannot be interpreted, i.e. UNKNOWN.
      if (row && row.trading_halted === true) {
        _cache = {
          state: 'HALTED',
      ...
      } else if (row && row.trading_halted === false) {
        _cache = {
          state: 'RUNNING',
          reason: row.reason ?? null,
          updatedBy: row.updated_by ?? null,
        ...
        _cache = unknownOrLatched('returned no interpretable row');
      }
    } catch (e) {
      _cache = unknownOrLatched(`lookup failed (${e.message})`);
    }
    _cacheAt = t;
    return _cache;
  }
  ...
  /** BigQuery optimistic-concurrency abort (concurrent update to the table). */
  function isConcurrentAbort(e) {
    return /concurrent|aborted|serializ|conflict/i.test(String(e?.message || e));
  }

  /**
   * Verify and consume a single-use approval token.
   *
   * Atomicity under BigQuery snapshot isolation: pure appends do NOT
   * conflict between concurrent transactions, so `INSERT ... WHERE NOT
   * EXISTS` alone cannot prevent two claimants from both committing a
   * 'USED' row. The mutex is the conditional UPDATE on the token's
   * `ISSUED` row — concurrent transactions modifying the same row DO
   * conflict, so at most one claimant commits. The winner stamps a unique
   * `claim:<uuid>` into the ISSUED row's `order_id` and appends a 'USED'
   * audit event; the loser aborts, retries, then sees `order_id` already
   * claimed, updates 0 rows, and its read-back fails the claim check.
   * The 'USED' event row remains append-only; the ISSUED row is mutated
   * exactly once, at consumption (NEEDS_JUN_DECISION: this deviates from
   * the strictly append-only model in order-approvals.md).
  ...
  async function verifyApproval({ token, symbol, side, qty }) {
    ...
    // let the same approval be replayed. The UPDATE on the ISSUED row is the
    // mutex: concurrent transactions modifying the same row conflict and
    // abort, so at most one claimant wins. The conditional USED insert and
    // the claim read-back then confirm we are that claimant.
      ...
      try {
        await bqQuery(
          `BEGIN TRANSACTION;
           ...
           SET order_id = @claim
           WHERE token = @token AND event = 'ISSUED' AND order_id IS NULL
             AND NOT EXISTS (
               SELECT 1 FROM ${APPROVALS_TABLE}
           ...
           SELECT @token, 'USED', @symbol, @side, @qty, @created_by, @claim, NULL, CURRENT_TIMESTAMP()
           FROM (SELECT 1)
           WHERE EXISTS (
             SELECT 1 FROM ${APPROVALS_TABLE}
             WHERE token = @token AND event = 'ISSUED' AND order_id = @claim
           );
           COMMIT TRANSACTION;`,
          params
        );
        ...
        if (attempt < CONSUME_MAX_ATTEMPTS && isConcurrentAbort(e)) {
          console.warn(`[GATE] approval consume aborted by a concurrent transaction (attempt ${attempt}); retrying`);
          continue;
        }
        throw e;
      }
    ...
    // Confirm OUR claim holds the token. Only the winning transaction's
    // UPDATE sets the ISSUED row's order_id to our claim; a concurrent or
    // previously-committed claim (or a legacy USED event row) leaves it
    // different — fail closed.
    const issuedAfter = await bqQuery(
      `SELECT order_id FROM ${APPROVALS_TABLE}
       WHERE token = @token AND event = 'ISSUED'
       ORDER BY created_at DESC
       LIMIT 1`,
      { token }
    );
    const claimRow = issuedAfter && issuedAfter[0];
    if (!claimRow) {
      return { ok: false, reason: 'approval consumption unconfirmed' };
    }
    if (claimRow.order_id !== claim) {
      return { ok: false, reason: 'approval token already used' };
    }
    return { ok: true };
  }
  ...
  async function checkOrder({ symbol, side, qty, approvalToken, source, idToken, idTokenPlatformVerified = false }) {
    ...
    // caller-supplied and spoofable, so it is honored only on explicit
    // operator opt-in while no allowlist is configured (transition mode).
    if (_trustedEmails.size) {
      if (await trustedCaller(idToken, idTokenPlatformVerified)) return ALLOW;
    } else if (allowLegacySourceLabel && source === 'magi-core') {
