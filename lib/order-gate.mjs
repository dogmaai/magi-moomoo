 ...
 * @param {(idToken: string) => Promise<object>} [deps.verifyIdToken] - verifies
 *   a Google-signed ID token and returns its claims (throws on invalid token)
...
export function createOrderGate({ bqQuery, getPositions, trustedCallerEmails = [], verifyIdToken = null, now = () => Date.now() }) {
   ...
   */
  async function trustedCaller(idToken) {
  ...
  /**
   * @param {{symbol: string, side: string, qty: number, approvalToken?: string, source?: string, idToken?: string}} order
   * @returns {Promise<{allow: boolean, status: number, code?: string, reason?: string}>}
   */
  async function checkOrder({ symbol, side, qty, approvalToken, source, idToken }) {
    const s = String(side || '').toUpperCase();
    const q = Number(qty);
    ...
    if (_trustedEmails.size) {
      if (await trustedCaller(idToken)) return ALLOW;
