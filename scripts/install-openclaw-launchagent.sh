#!/bin/bash
# install-openclaw-launchagent.sh — Install a macOS LaunchAgent for the OpenClaw named tunnel.
#
...
#
# Usage:
#   bash scripts/install-openclaw-launchagent.sh [hostname]
#
# Example:
...
HOSTNAME="${1:-openclaw.khaos.company}"
...
TOKEN_SECRET="OPENCLAW_TUNNEL_TOKEN"

CLOUDFLARED_DIR="${HOME}/.cloudflared"
PLIST_NAME="com.magi.openclaw.cloudflared"
PLIST_SRC="scripts/com.magi.openclaw.cloudflared.plist"
...
if [ -z "${TOKEN}" ]; then
  ...
  echo "         Run setup-openclaw-named-tunnel.sh first and store the printed token:"
  echo "         echo -n '<token>' | gcloud secrets versions add ${TOKEN_SECRET} --project=${PROJECT_ID} --data-file=-"
  exit 1
fi
...
echo "  Logs:  /tmp/magi-openclaw-cloudflared.stdout.log"
echo "         /tmp/magi-openclaw-cloudflared.stderr.log"
